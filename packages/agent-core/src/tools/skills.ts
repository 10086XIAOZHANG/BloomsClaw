...
export const loadSkillTool = tool(
  async (input: { skill_name: string }) => {
    ...
    if (!name) {
      const available = listActiveSkills().map((s) => s.name).join(', ');
...
export const readSkillResourceTool = tool(
  async (input: { skill_name: string; relative_path: string }) => {
    ...
    const result = readSkillResourceFile(name, rel);
    if (!result.ok || result.content == null) {
      return result.error ?? `读取失败: ${rel}`;
    }
    const byteLen = fs.existsSync(path.join(getSkillDir(name), rel))
      ? fs.statSync(path.join(getSkillDir(name), rel)).size
